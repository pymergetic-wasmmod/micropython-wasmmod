...
pub fn count_params(emit: *mut EmitInlineAsm, n_params: usize, pn_params: *mut ParseNode) -> usize {
    ...
    if n_params > 4 {
        error_msg(
            unsafe { &mut *emit },
            b"can only have up to 4 parameters to Thumb assembly",
        );
        return 0;
    }
    for i in 0..n_params {
        ...
        if !parse::parse_node_is_id(pn) {
            error_msg(
                unsafe { &mut *emit },
                b"parameters must be registers in sequence r0 to r3",
            );
            return 0;
        }
...
const FORMAT_4_OP_TABLE: &[Format4Op] = &[
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_EOR),
        name: [b'e', b'o', b'r'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_LSL),
        name: [b'l', b's', b'l'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_LSR),
        name: [b'l', b's', b'r'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_ASR),
        name: [b'a', b's', b'r'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_ADC),
        name: [b'a', b'd', b'c'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_SBC),
        name: [b's', b'b', b'c'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_ROR),
        name: [b'r', b'o', b'r'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_TST),
        name: [b't', b's', b't'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_NEG as u32),
        name: [b'n', b'e', b'g'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_CMP as u32),
        name: [b'c', b'm', b'p'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_CMN as u32),
        name: [b'c', b'm', b'n'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_ORR as u32),
        name: [b'o', b'r', b'r'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_MUL as u32),
        name: [b'm', b'u', b'l'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_BIC as u32),
        name: [b'b', b'i', b'c'],
    },
    Format4Op {
        op: x(asmthumb::ASM_THUMB_FORMAT_4_MVN as u32),
        name: [b'm', b'v', b'n'],
    },
];
...
const FORMAT_9_10_OP_TABLE: &[Format910Op] = &[
    Format910Op {
        op: asmthumb::ASM_THUMB_FORMAT_9_LDR | asmthumb::ASM_THUMB_FORMAT_9_WORD_TRANSFER,
        name: b"ldr",
    },
    Format910Op {
        op: asmthumb::ASM_THUMB_FORMAT_9_LDR | asmthumb::ASM_THUMB_FORMAT_9_BYTE_TRANSFER,
        name: b"ldrb",
    },
    Format910Op {
        op: asmthumb::ASM_THUMB_FORMAT_10_LDRH,
        name: b"ldrh",
    },
    Format910Op {
        op: asmthumb::ASM_THUMB_FORMAT_9_STR | asmthumb::ASM_THUMB_FORMAT_9_WORD_TRANSFER,
        name: b"str",
    },
    Format910Op {
        op: asmthumb::ASM_THUMB_FORMAT_9_STR | asmthumb::ASM_THUMB_FORMAT_9_BYTE_TRANSFER,
        name: b"strb",
    },
    Format910Op {
        op: asmthumb::ASM_THUMB_FORMAT_10_STRH,
        name: b"strh",
    },
];
            ...
            if r.reg as usize > max_reg {
                error_exc(
                    ...
                    objexcept::new_exception_args(
                        objexcept::type_syntax_error(),
                        1,
                        &[objstr::new_str(b"register out of range")],
                    ),
                );
                return 0;
            }
...
fn get_arg_special_reg(emit: &mut EmitInlineAsm, _op: &[u8], pn: ParseNode) -> usize {
    ...
    error_exc(
        ...
        objexcept::new_exception_args(
            objexcept::type_syntax_error(),
            1,
            &[objstr::new_str(b"expects a special register")],
        ),
    );
    0
}
        ...
        if regno > 31 {
            error_exc(
                ...
                objexcept::new_exception_args(
                    objexcept::type_syntax_error(),
                    1,
                    &[objstr::new_str(b"vfp register out of range")],
                ),
            );
            return 0;
        }
...
fn goto_malformed_vfp(emit: &mut EmitInlineAsm) {
    error_exc(
        ...
        objexcept::new_exception_args(
            objexcept::type_syntax_error(),
            1,
            &[objstr::new_str(b"expects an FPU register")],
        ),
    );
}
...
fn goto_bad_reglist(emit: &mut EmitInlineAsm, _op: &[u8]) {
    error_exc(
        ...
        objexcept::new_exception_args(
            objexcept::type_syntax_error(),
            1,
            &[objstr::new_str(b"expects {r0, r1, ...}")],
        ),
    );
}
    ...
    if (i & !fit_mask) != 0 {
        error_exc(
            ...
            objexcept::new_exception_args(
                objexcept::type_syntax_error(),
                1,
                &[objstr::new_str(b"integer doesn't fit in mask")],
            ),
        );
        return 0;
    }
...
fn goto_bad_addr(emit: &mut EmitInlineAsm, _op: &[u8]) {
    error_exc(
        ...
        objexcept::new_exception_args(
            objexcept::type_syntax_error(),
            1,
            &[objstr::new_str(b"expects an address of the form [a, b]")],
        ),
    );
}
...
fn emit_vfp_op(
    ...
    if n_args == 2 {
        ...
        } else if qstr_eq_name(op, b"vsqrt")
            || qstr_eq_name(op, b"vcvt_f32_s32")
            || qstr_eq_name(op, b"vcvt_s32_f32")
        {
            let hi = if qstr_eq_name(op, b"vsqrt") {
                0xeeb1
            } else if qstr_eq_name(op, b"vcvt_f32_s32") {
                0xeeb8
            } else {
                0xeebd
            };
            ...
            let reg_dest = if reg_str0.starts_with(b"APSR_nzcv") {
                15
            } else {
                get_arg_reg(emit, op_str, unsafe { *pn_args }, 15)
            };
            ...
            let (op_code_hi, r_arm, vm) = if reg_str.first() == Some(&b'r') {
                (
                    0xee10,
                    get_arg_reg(emit, op_str, unsafe { *pn_args }, 15),
                    get_arg_vfpreg(emit, op_str, unsafe { *pn_args.add(1) }),
                )
            } else {
                (
                    0xee00,
                    get_arg_reg(emit, op_str, unsafe { *pn_args.add(1) }, 15),
                    get_arg_vfpreg(emit, op_str, unsafe { *pn_args }),
                )
            };
            asmthumb::op32(
                &mut emit.as_,
                op_code_hi | (((vm as u32) & 0x1e) >> 1),
                0x0a10 | ((r_arm as u32) << 12) | (((vm as u32) & 1) << 7),
            );
        } else if qstr_eq_name(op, b"vldr") || qstr_eq_name(op, b"vstr") {
            let op_code_hi = if qstr_eq_name(op, b"vldr") {
                0xed90
            } else {
                0xed80
            };
            let vd = get_arg_vfpreg(emit, op_str, unsafe { *pn_args });
            if let Some((pn_base, pn_offset)) =
                get_arg_addr(emit, op_str, unsafe { *pn_args.add(1) })
            {
                let rlo_base = get_arg_reg(emit, op_str, pn_base, 7);
                let i8 = get_arg_i(emit, op_str, pn_offset, 0x3fc) >> 2;
                asmthumb::op32(
                    &mut emit.as_,
                    op_code_hi | (rlo_base as u32),
                    0x0a00 | (((vd as u32) & 0x1e) << 11) | (i8 as u32),
                );
            }
        } else {
            unknown_op(emit, op_str, 2);
        ...
        for entry in FORMAT_VFP_OP_TABLE {
            if op_str.len() >= 4
                && op_str[1..4] == entry.name
                && op_str.get(4).copied().unwrap_or(0) == 0
            {
                ...
                asmthumb::op32(
                    &mut emit.as_,
                    op_code_hi | (((vd as u32) & 1) << 6) | ((vn as u32) >> 1),
                    op_code
                        | ((vm as u32) >> 1)
                        | (((vm as u32) & 1) << 5)
                        | (((vd as u32) & 0x1e) << 11)
...
fn emit_one_arg(
    ...
    if qstr_eq_name(op, b"b") {
        ...
        && (op_len == 3
            || (op_len == 5
                && op_str[3] == b'_'
                && (op_str[4] == b'n' || (asmthumb::allow_armv7m(&emit.as_) && op_str[4] == b'w'))))
    {
        ...
        let arg_slice = if arg_str.last() == Some(&0) {
...
fn emit_two_args(emit: &mut EmitInlineAsm, op: Qstr, op_str: &[u8], pn_args: *mut ParseNode) {
    if parse::parse_node_is_id(unsafe { *pn_args.add(1) }) {
        if qstr_eq_name(op, b"mov") {
            ...
            && (qstr_eq_name(op, b"clz") || qstr_eq_name(op, b"rbit"))
        {
            let (hi, lo) = if qstr_eq_name(op, b"clz") {
                (0xfab0, 0xf080)
            } else {
                (0xfa90, 0xf0a0)
            };
            ...
            asmthumb::op32(
                &mut emit.as_,
                0xf3ef,
                0x8000 | ((reg_dest as u32) << 8) | (reg_src as u32),
            );
            ...
            asmthumb::format_4(
                &mut emit.as_,
                asmthumb::ASM_THUMB_FORMAT_4_AND,
                reg_dest as u32,
                reg_src as u32,
            );
        } else {
            for entry in FORMAT_4_OP_TABLE {
                if op_str.len() >= 3
                    && op_str[..3] == entry.name[..3]
                    && op_str.get(3).copied().unwrap_or(0) == 0
                {
                    let reg_dest = get_arg_reg(emit, op_str, unsafe { *pn_args }, 7);
                    let reg_src = get_arg_reg(emit, op_str, unsafe { *pn_args.add(1) }, 7);
                    asmthumb::format_4(
                        &mut emit.as_,
                        0x4000 | ((entry.op as u16) << 4),
                        reg_dest as u32,
                        reg_src as u32,
                    );
                    return;
        ...
        asmthumb::format_3(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_3_MOV,
            rlo_dest as u32,
            i8_src as i32,
        );
        ...
        asmthumb::format_3(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_3_CMP,
            rlo_dest as u32,
            i8_src as i32,
        );
        ...
        asmthumb::format_3(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_3_ADD,
            rlo_dest as u32,
            i8_src as i32,
        );
        ...
        asmthumb::format_3(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_3_SUB,
            rlo_dest as u32,
            i8_src as i32,
        );
    } else if asmthumb::allow_armv7m(&emit.as_)
        && (qstr_eq_name(op, b"movw") || qstr_eq_name(op, b"movt"))
    {
        let reg_dest = get_arg_reg(emit, op_str, unsafe { *pn_args }, 15);
        let i_src = get_arg_i(emit, op_str, unsafe { *pn_args.add(1) }, 0xffff) as i32;
        let mov_op = if qstr_eq_name(op, b"movw") {
            asmthumb::ASM_THUMB_OP_MOVW
        } else {
            asmthumb::ASM_THUMB_OP_MOVT
        };
        ...
        if let Some((pn_base, pn_offset)) = get_arg_addr(emit, op_str, unsafe { *pn_args.add(1) }) {
            ...
            asmthumb::op32(
                &mut emit.as_,
                0xe850 | (r_base as u32),
                0x0f00 | ((r_dest as u32) << 12) | i8,
            );
        }
    } else {
...
fn emit_three_args(emit: &mut EmitInlineAsm, op: Qstr, op_str: &[u8], pn_args: *mut ParseNode) {
    if qstr_eq_name(op, b"lsl") {
        ...
        asmthumb::format_1(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_1_LSL,
            rlo_dest as u32,
            rlo_src as u32,
            i5,
        );
        ...
        asmthumb::format_1(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_1_LSR,
            rlo_dest as u32,
            rlo_src as u32,
            i5,
        );
        ...
        asmthumb::format_1(
            &mut emit.as_,
            asmthumb::ASM_THUMB_FORMAT_1_ASR,
            rlo_dest as u32,
            rlo_src as u32,
            i5,
        );
    } else if qstr_eq_name(op, b"add") || qstr_eq_name(op, b"sub") {
        let base_op = if qstr_eq_name(op, b"add") {
            asmthumb::ASM_THUMB_FORMAT_2_ADD
        } else {
            asmthumb::ASM_THUMB_FORMAT_2_SUB
        };
        ...
        && (qstr_eq_name(op, b"sdiv") || qstr_eq_name(op, b"udiv"))
    {
        let op_code = if qstr_eq_name(op, b"sdiv") {
            0xfb90
        } else {
            0xfbb0
        };
...
fn unknown_op(emit: &mut EmitInlineAsm, _op_str: &[u8], _n_args: usize) {
    error_exc(
        ...
        objexcept::new_exception_args(
            objexcept::type_syntax_error(),
            1,
            &[objstr::new_str(b"unsupported Thumb instruction")],
        ),
    );
}
