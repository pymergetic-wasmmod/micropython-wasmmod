...
fn mpn_shl(idig: &mut [Dig], jdig: &[Dig], jlen: usize, n: Uint) -> usize {
    ...
    if n_part == 0 { n_part = DIG_SIZE as usize; }
...
fn mpn_shr(idig: &mut [Dig], jdig: &[Dig], jlen: usize, n: Uint) -> usize {
    ...
    if jlen2 > 0 && idig[jlen2 - 1] == 0 { jlen2 - 1 } else { jlen2 }
}
...
fn mpn_and(idig: &mut [Dig], jdig: &[Dig], kdig: &[Dig], klen: usize) -> usize {
    for i in 0..klen { idig[i] = jdig[i] & kdig[i]; }
    mpn_remove_trailing_zeros(idig, klen)
}

fn mpn_and_neg(
    idig: &mut [Dig], jdig: &[Dig], jlen: usize, kdig: &[Dig], klen: usize,
    mut carryi: DblDig, mut carryj: DblDig, mut carryk: DblDig,
) -> usize {
...
fn mpn_or(idig: &mut [Dig], jdig: &[Dig], jlen: usize, kdig: &[Dig], klen: usize) -> usize {
    let extra = jlen - klen;
    for i in 0..klen { idig[i] = jdig[i] | kdig[i]; }
    for i in 0..extra { idig[klen + i] = jdig[klen + i]; }
    jlen
}

fn mpn_or_neg(
    idig: &mut [Dig], jdig: &[Dig], jlen: usize, kdig: &[Dig], klen: usize,
    mut carryj: DblDig, mut carryk: DblDig,
) -> usize {
...
fn mpn_div(
    num_dig: &mut [Dig], num_len: &mut usize,
    den_dig: &[Dig], den_len: usize,
    quo_dig: &mut [Dig], quo_len: &mut usize,
) {
    ...
    *quo_len = *num_len - den_len;
    ...
    for q in (0..*quo_len).rev() {
        let mut quo: DblDig = ((num_dig[num_idx] as DblDig) << DIG_SIZE) | num_dig[num_idx - 1] as DblDig;
        quo /= lead_den.max(1);
        let mut borrow: DblDigSigned = 0;
        ...
        for j in 0..den_len {
            ...
            let low = (borrow & DIG_MASK as DblDigSigned) as DblDig + num_dig[n_idx] as DblDig - (x & DIG_MASK as DblDig);
            num_dig[n_idx] = (low & DIG_MASK as DblDig) as Dig;
            borrow = (borrow >> DIG_SIZE as i32) - (x >> DIG_SIZE) as DblDigSigned + (low >> DIG_SIZE) as DblDigSigned;
        }
    ...
    while *quo_len > 0 && quo_dig[*quo_len - 1] == 0 { *quo_len -= 1; }
...
pub fn init_zero(z: &mut Mpz) {
    *z = Mpz { neg: false, fixed_dig: false, alloc: 0, len: 0, dig: Vec::new() };
}
...
fn mpz_clone(src: &Mpz) -> Mpz {
    ...
    let mut z = Mpz { neg: src.neg, fixed_dig: false, alloc: src.alloc.max(MIN_ALLOC), len: src.len, dig: vec![0; src.alloc.max(MIN_ALLOC)] };
...
pub fn set_from_int(z: &mut Mpz, val: Int) {
    ...
    let (neg, mut uval) = if val < 0 { (true, (-val) as u64) } else { (false, val as u64) };
    z.neg = neg;
    z.len = 0;
    while uval > 0 {
...
pub fn set_from_bytes(z: &mut Mpz, big_endian: bool, buf: &[Byte]) {
    need_dig(z, (buf.len() * 8 + DIG_SIZE as usize - 1) / DIG_SIZE as usize);
    let mut d: Dig = 0;
...
pub fn is_zero(z: &Mpz) -> bool { z.len == 0 }
pub fn is_neg(z: &Mpz) -> bool { z.neg }
...
pub fn abs_inpl(dest: &mut Mpz, src: &Mpz) {
    if !std::ptr::eq(dest, src) { set(dest, src); }
    dest.neg = false;
}

pub fn neg_inpl(dest: &mut Mpz, src: &Mpz) {
    if !std::ptr::eq(dest, src) { set(dest, src); }
    if dest.len > 0 { dest.neg = !dest.neg; }
}
...
pub fn shl_inpl(dest: &mut Mpz, lhs: &Mpz, rhs: u32) {
    if lhs.len == 0 || rhs == 0 {
        ...
        need_dig(dest, lhs.len + (rhs as usize + DIG_SIZE as usize - 1) / DIG_SIZE as usize);
    ...
    if lhs.len == 0 || rhs == 0 {
        ...
        if dest.neg {
            ...
            for i in 0..lhs.len.min(n_whole) {
                if lhs.dig[i] != 0 { round_up = 1; break; }
            }
...
pub fn and_inpl(dest: &mut Mpz, lhs: &Mpz, rhs: &Mpz) {
    let (lhs, rhs) = if lhs.len < rhs.len { (rhs, lhs) } else { (lhs, rhs) };
    if mpconfig::OPT_MPZ_BITWISE && !lhs.neg && !rhs.neg {
        need_dig(dest, lhs.len);
        if mpconfig::OPT_MPZ_BITWISE {
            for i in 0..rhs.len { dest.dig[i] = lhs.dig[i] & rhs.dig[i]; }
        ...
        dest.len = mpn_and_neg(
            &mut dest.dig, &lhs.dig[..lhs.len], lhs.len, &rhs.dig[..rhs.len], rhs.len,
            if lhs.neg == rhs.neg { lhs.neg as DblDig } else { 0 },
            lhs.neg as DblDig, rhs.neg as DblDig,
        );
...
pub fn or_inpl(dest: &mut Mpz, lhs: &Mpz, rhs: &Mpz) {
    ...
    if mpconfig::OPT_MPZ_BITWISE && !lhs.neg && !rhs.neg {
        need_dig(dest, lhs.len);
        for i in 0..rhs.len { dest.dig[i] = lhs.dig[i] | rhs.dig[i]; }
        for i in rhs.len..lhs.len { dest.dig[i] = lhs.dig[i]; }
        dest.len = lhs.len;
        dest.neg = false;
    } else if mpconfig::OPT_MPZ_BITWISE {
        need_dig(dest, lhs.len + 1);
        dest.len = mpn_or_neg(&mut dest.dig, &lhs.dig[..lhs.len], lhs.len, &rhs.dig[..rhs.len], rhs.len, lhs.neg as DblDig, rhs.neg as DblDig);
        dest.neg = true;
    } else {
...
pub fn xor_inpl(dest: &mut Mpz, lhs: &Mpz, rhs: &Mpz) {
    ...
    if mpconfig::OPT_MPZ_BITWISE && lhs.neg == rhs.neg {
        ...
        dest.len = if !lhs.neg {
            ...
            mpn_xor_neg(&mut dest.dig, &lhs.dig[..lhs.len], lhs.len, &rhs.dig[..rhs.len], rhs.len, 0, 0, 0)
        };
        dest.neg = false;
    } else if mpconfig::OPT_MPZ_BITWISE {
        need_dig(dest, lhs.len + 1);
        dest.len = mpn_xor_neg(
            &mut dest.dig, &lhs.dig[..lhs.len], lhs.len, &rhs.dig[..rhs.len], rhs.len,
            1, if !lhs.neg { 1 } else { 0 }, if !rhs.neg { 1 } else { 0 },
        );
        dest.neg = true;
    } else {
        need_dig(dest, lhs.len + if lhs.neg || rhs.neg { 1 } else { 0 });
        dest.len = mpn_xor_neg(
            ...
            if !lhs.neg { 1 } else { 0 }, if !rhs.neg { 1 } else { 0 },
        );
        dest.neg = lhs.neg ^ rhs.neg;
    }
}
...
pub fn mul_inpl(dest: &mut Mpz, lhs: &Mpz, rhs: &Mpz) {
    ...
    if std::ptr::eq(lhs, dest) {
        ...
        if std::ptr::eq(rhs, dest) { rhs = lhs; }
    ...
    dest.dig.fill(0);
    dest.len = mpn_mul(&mut dest.dig, &lhs.dig[..lhs.len], lhs.len, &rhs.dig[..rhs.len], rhs.len);
    dest.neg = lhs.neg != rhs.neg;
}
...
pub fn pow3_inpl(dest: &mut Mpz, lhs: &Mpz, rhs: &Mpz, modulus: &Mpz) {
    ...
    let mut x = mpz_clone(lhs);
    let mut n = mpz_clone(rhs);
    let mut quo = Mpz { neg: false, fixed_dig: false, alloc: 0, len: 0, dig: Vec::new() };
    init_zero(&mut quo);
    while n.len > 0 {
        if (n.dig[0] & 1) != 0 {
...
pub fn divmod_inpl(dest_quo: &mut Mpz, dest_rem: &mut Mpz, lhs: &Mpz, rhs: &Mpz) {
    ...
    let mut qlen = 0usize;
    mpn_div(
        &mut dest_rem.dig, &mut dest_rem.len,
        &rhs_copy, rhs_copy.len(),
        &mut dest_quo.dig, &mut qlen,
    );
    dest_quo.len = qlen;
    ...
    if lhs_copy.len() > 0 && lhs.neg != rhs.neg {
        ...
        if !is_zero(dest_rem) {
            ...
            let q = Mpz { neg: dest_quo.neg, fixed_dig: false, alloc: quo_tmp.len(), len: quo_tmp.len(), dig: quo_tmp };
            add_inpl(dest_quo, &q, &m1);
            let rem_tmp = dig_copy(dest_rem);
            let r = Mpz { neg: dest_rem.neg, fixed_dig: false, alloc: rem_tmp.len(), len: rem_tmp.len(), dig: rem_tmp };
...
pub fn hash(z: &Mpz) -> Int {
    ...
    if z.neg { -(val as Int) } else { val as Int }
}

pub fn as_int_checked(z: &Mpz, value: &mut Int) -> bool {
    let mut val: Uint = 0;
    for &d in z.dig[..z.len].iter().rev() {
        if val > (!(obj::WORD_MSBIT_HIGH) >> DIG_SIZE as usize) { return false; }
        val = (val << DIG_SIZE) | d as Uint;
    }
    if z.neg { val = val.wrapping_neg(); }
    *value = val as Int;
    true
}

pub fn as_uint_checked(z: &Mpz, value: &mut Uint) -> bool {
    if z.neg { return false; }
    let mut val: Uint = 0;
    for &d in z.dig[..z.len].iter().rev() {
        if val > (!(obj::WORD_MSBIT_HIGH) >> (DIG_SIZE as usize - 1)) { return false; }
        val = (val << DIG_SIZE) | d as Uint;
    }
    *value = val;
    true
...
pub fn as_bytes(z: &Mpz, big_endian: bool, as_signed: bool, len: usize, buf: &mut [Byte]) -> bool {
    ...
    while zidx < z.len {
        ...
        while bits >= 8 {
            ...
            if olen == 0 {
                if val as Byte != fill_byte { return false; }
                continue;
            }
            if big_endian { buf[len - olen] = val as Byte; } else { buf[len - olen] = val as Byte; }
            olen -= 1;
        }
    }
...
pub fn as_float(z: &Mpz) -> f64 {
    ...
    if z.neg { -val } else { val }
}

pub fn max_num_bits(z: &Mpz) -> usize { z.len * DIG_SIZE as usize }

pub fn as_str_inpl(z: &Mpz, base: u32, prefix: Option<&str>, base_char: u8, comma: u8, out: &mut [u8]) -> usize {
    debug_assert!((2..=32).contains(&base));
    let ilen = z.len;
    if ilen == 0 {
        let mut pos = 0usize;
        if let Some(p) = prefix { pos += p.len(); out[..pos].copy_from_slice(p.as_bytes()); }
        out[pos] = b'0';
        return pos + 1;
    }
    ...
    loop {
        ...
        if ch > b'9' { ch = ch - 10 + base_char; }
        out[s] = ch;
        s += 1;
        for d in &dig {
            if *d != 0 { done = false; break; }
        }
        if done { break; }
    ...
    if let Some(p) = prefix {
        for c in p.as_bytes().iter().rev() { out[s] = *c; s += 1; }
    }
    if z.neg { out[s] = b'-'; s += 1; }
    out[..s].reverse();
    s
}
